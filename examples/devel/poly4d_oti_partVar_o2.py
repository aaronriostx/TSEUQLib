...
rv_pdf_name = np.array(['U','U','U','U']) # PDF of each variable
rv_a = np.array([1,1,2,2]) # shape parameter of each variable
rv_b = np.array([3+oti.e(21,order=1),8,6,4]) # scale parameter of each variable
...
oti_mu = moti.oti_moment(n_dim)


# Expectation and Variance estimates

mu_y   = oti_mu.oti_expectation(y,rv_mu_joint)

# Extract sensitivities of central moments wrt rv parameters
dev_db1 = mu_y.get_deriv([21])
print('dev_db1 =', dev_db1)
input('i')

Vy     = oti_mu.oti_central_moment(y,2,rv_mu_joint)
dVy_db1 = Vy.get_deriv([21])
print('dVy_db1 =', dVy_db1)
input('i')

# Third and fourth moment estimates.
muc_y3 = oti_mu.oti_central_moment(y,3,rv_mu_joint)
muc_y4 = oti_mu.oti_central_moment(y,4,rv_mu_joint)
