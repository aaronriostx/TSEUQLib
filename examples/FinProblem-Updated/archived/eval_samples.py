...
if __name__ == '__main__':
    ...
    order = 4   # Order of derivative to be computed.
    t_max = 950 # Total seconds to evaluate.
    ...
    oti.save(Tanaly,"export_solutions/Tx_end_nominal_n{0:d}.matso".format(order))
    
    # # You can read it back using 
    # # >>> Tanaly = oti.read(<filename>)
    # print(" "*100,end='\r')
    # print("Total run time {0:.6f} s ".format( end_time-start_time ) )
    
    # Nominal plot.
    #
    # plt.figure()
    # plt.plot(t,Tanaly.real)
    # plt.xlabel('time [s]')
    ...
    # These are the perturbations used for all runs in the finite element 
    # analises and other calculations.
    for i_sampl in range(50):#samples.shape[0]):

        k    = samples[i_sampl,0] + oti.e(1,order=order)
        Cp   = samples[i_sampl,1] + oti.e(2,order=order)
        rho  = samples[i_sampl,2] + oti.e(3,order=order)
        
        print(f'k: {k.real} Cp: {Cp.real} rho: {rho.real}')
        ...
        plt.plot(t,Tanaly.real,'C1',alpha=0.5)

        oti.save(Tanaly,f"export_solutions/T_tip_{i_sampl+1:03}_n{order:d}.matso")


    # end for 

    # print(f'k: {k.real} Cp: {Cp.real} rho: {rho.real}')
    # k    = k.real   + oti.e(1,order=order) # [3-20] is a maximum range.
    # Cp   = Cp.real  + oti.e(2,order=order) # nom: 580 [200-1000]
    # rho  = 8000.0   + oti.e(3,order=order) # nom: 4430 [1500-8000]
    ...
    #     Ti,Th_i,Th_ti,Th_ssi = analytic_solution(x, ti, b, d, rho, Cp, dt, k,
    #                                             hu, T0, Tw, Tinf, 
    #                                             base = oti, nterms = 1000 )
    #     Tanaly[i,:] = Ti
