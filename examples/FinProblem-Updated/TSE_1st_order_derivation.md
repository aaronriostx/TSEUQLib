...
- **Epistemic parameters**: $\mathbf{y} = [T_\infty,\, T_W,\, b]$, known only to lie within intervals, with midpoints $\mathbf{y}_0$. Since $T_0 = T_\infty$ by assumption, $T_0$ is coupled to $T_\infty$ and is not an independent variable.
...
$$
(\boldsymbol{\mu}_X, \mathbf{y}_0), \qquad Z = 0, \qquad y_{0,j} = \frac{\underline{y}_j + \overline{y}_j}{2}
$$
...
$$
T(\mathbf{X}, \mathbf{y}, \tau) \;\approx\; T(\boldsymbol{\mu}_X, \mathbf{y}_0, \tau) \;+\; \nabla_{\mathbf{X}} T\big|_{\text{nom}} \cdot (\mathbf{X} - \boldsymbol{\mu}_X) \;+\; \nabla_{\mathbf{y}} T\big|_{\text{nom}} \cdot (\mathbf{y} - \mathbf{y}_0)
$$
...
$$
T \approx \underbrace{T(\boldsymbol{\mu}_X, \mathbf{y}_0, \tau)}_{T_0(\tau)}
...
$$
+\; \frac{\partial T}{\partial T_\infty}\bigg|_{\text{nom}}\!(T_\infty - T_{\infty,0})
+ \frac{\partial T}{\partial T_W}\bigg|_{\text{nom}}\!(T_W - T_{W,0})
+ \frac{\partial T}{\partial b}\bigg|_{\text{nom}}\!(b - b_0)
$$
...
Write $\delta k = k - \mu_k$, etc., for the aleatory deviations, and similarly for $\delta C_p, \delta\rho, \delta T_\infty, \delta T_W, \delta b$:

$$
T \approx T_0(\tau) + \left(\dfrac{\partial T}{\partial k}\bigg|_{\text{nom}}\right)\,\delta k + \left(\dfrac{\partial T}{\partial C_p}\bigg|_{\text{nom}}\right)\,\delta C_p + \left(\dfrac{\partial T}{\partial \rho}\bigg|_{\text{nom}}\right)\,\delta\rho + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)\,\sigma_{h_U}\, Z + \left(\dfrac{\partial T}{\partial T_\infty}\bigg|_{\text{nom}}\right)\,\delta T_\infty + \left(\dfrac{\partial T}{\partial T_W}\bigg|_{\text{nom}}\right)\,\delta T_W + \left(\dfrac{\partial T}{\partial b}\bigg|_{\text{nom}}\right)\,\delta b
$$
...
$$
\mathbb{E}_{\mathbf{X}}[T](\mathbf{y}) \approx T_0(\tau) + \left(\dfrac{\partial T}{\partial T_\infty}\bigg|_{\text{nom}}\right)\,\delta T_\infty + \left(\dfrac{\partial T}{\partial T_W}\bigg|_{\text{nom}}\right)\,\delta T_W + \left(\dfrac{\partial T}{\partial b}\bigg|_{\text{nom}}\right)\,\delta b
$$
...
$$
\mathbb{E}[T(\tau)] \in T_0(\tau) \;\pm\; \sum_{j} \left|\frac{\partial T}{\partial y_j}\bigg|_{\text{nom}}\right|\, \Delta y_j
$$
...
$$
\mathrm{Var}_{\mathbf{X}}[T] \approx \left(\dfrac{\partial T}{\partial k}\bigg|_{\text{nom}}\right)^2 \sigma_k^2 + \left(\dfrac{\partial T}{\partial C_p}\bigg|_{\text{nom}}\right)^2 \sigma_{C_p}^2 + \left(\dfrac{\partial T}{\partial \rho}\bigg|_{\text{nom}}\right)^2 \sigma_\rho^2 + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\, \sigma_{h_U}^2
$$
...
$$
\mathrm{Var}[T(\tau)] \in \Big[\left(\dfrac{\partial T}{\partial k}\bigg|_{\text{nom}}\right)^2 \sigma_k^2 + \left(\dfrac{\partial T}{\partial C_p}\bigg|_{\text{nom}}\right)^2 \sigma_{C_p}^2 + \left(\dfrac{\partial T}{\partial \rho}\bigg|_{\text{nom}}\right)^2 \sigma_\rho^2 + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\, \underline{\sigma}_{h_U}^2,\;\; \left(\dfrac{\partial T}{\partial k}\bigg|_{\text{nom}}\right)^2 \sigma_k^2 + \left(\dfrac{\partial T}{\partial C_p}\bigg|_{\text{nom}}\right)^2 \sigma_{C_p}^2 + \left(\dfrac{\partial T}{\partial \rho}\bigg|_{\text{nom}}\right)^2 \sigma_\rho^2 + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\, \overline{\sigma}_{h_U}^2\Big]
$$
...
$$
V_k = \left(\dfrac{\partial T}{\partial k}\bigg|_{\text{nom}}\right)^2\sigma_k^2, \qquad V_{C_p} = \left(\dfrac{\partial T}{\partial C_p}\bigg|_{\text{nom}}\right)^2\sigma_{C_p}^2, \qquad V_\rho = \left(\dfrac{\partial T}{\partial \rho}\bigg|_{\text{nom}}\right)^2\sigma_\rho^2, \qquad V_{h_U}(\sigma_{h_U}^2) = \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\,\sigma_{h_U}^2
$$
...
$$
S_{h_U}(x) = \frac{\left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2 x}{V_{\text{fixed}} + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2 x}
$$

Dividing the separately-computed intervals $V_{h_U} \in [\left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\underline{x}, \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\overline{x}]$ and $\mathrm{Var}[T] \in [\underline{\mathrm{Var}}[T], \overline{\mathrm{Var}}[T]]$ would pair the smallest numerator with the largest denominator (and vice versa) — combinations that can't actually co-occur, since both move together with $x$. Instead, note $S_{h_U}(x)$ is **monotonically increasing** in $x$ (for $V_{\text{fixed}} > 0$):

$$
\frac{dS_{h_U}}{dx} = \frac{\left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\, V_{\text{fixed}}}{\big(V_{\text{fixed}} + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2 x\big)^2} > 0
$$

so its exact range comes from evaluating **pointwise** at the two interval endpoints, not from interval division:

$$
S_{h_U} \in \left[\frac{\left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\,\underline{\sigma}_{h_U}^2}{V_{\text{fixed}} + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\,\underline{\sigma}_{h_U}^2},\;\; \frac{\left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\,\overline{\sigma}_{h_U}^2}{V_{\text{fixed}} + \left(\dfrac{\partial T}{\partial h_U}\bigg|_{\text{nom}}\right)^2\,\overline{\sigma}_{h_U}^2}\right]
$$
...
## Validation against the double-loop TSE: why $\mathbb{E}[T]$ matches but $\mathrm{Var}[T]$ does not

`double_loop_TSE_1st_order.py` builds a genuinely different object than the closed-form IA bounds above: at each point $\mathbf{y}$ on an explicit outer grid, it re-evaluates the aleatory gradient $f_i(\mathbf{y}) = \partial T/\partial X_i\big|_{(\boldsymbol\mu_X,\mathbf{y})}$ via fresh finite differences of the true model, rather than linearizing $f_i$'s dependence on $\mathbf{y}$ the way the single-expansion IA construction above does (which freezes $f_i$ at $\mathbf{y}_0$ and only lets $\mathbf{y}$ enter through the separately-computed epistemic gradient $g_j$). Comparing the two (the comparison appended to `TSE_1st_order_bounds.py` against `double_loop_TSE_1st_order_data.npz`) shows the $\mathbb{E}[T](\tau)$ bounds agree closely, while the $\mathrm{Var}[T](\tau)$ bounds diverge substantially. This isn't numerical noise — it follows from an exact structural fact about this model.

### $T$ is exactly affine in $(T_\infty, T_W)$

Since $T_0 = T_\infty$ by assumption, $\theta_0 = (T_0-T_\infty)/(T_W-T_\infty) \equiv 0$, so **$\theta$ has no dependence on $T_\infty$ or $T_W$ at all** — only on the "shape" variables $(b, k, C_p, \rho, h_U)$. The dimensional temperature is then

$$
T(\mathbf{X}, \mathbf{y}) = \theta(\mathbf{X}, b) \cdot (T_W - T_\infty) + T_\infty
$$

which is an **exact** affine function of $(T_\infty, T_W)$ for any fixed $(\mathbf{X}, b)$:

$$
\frac{\partial T}{\partial T_\infty} = 1-\theta, \qquad \frac{\partial T}{\partial T_W} = \theta, \qquad \frac{\partial^2 T}{\partial T_\infty^2}=\frac{\partial^2 T}{\partial T_W^2}=\frac{\partial^2 T}{\partial T_\infty \partial T_W}=0
$$
